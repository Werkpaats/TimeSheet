...
const spreadsheet = SpreadsheetApp.openById(
  "PLEASE ADD THE SPREADSHEET ID HERE"
);
...
function scheduledTask() {
  ...
  teachers.map(function (teacher, index) {
    ...
    let reminderDate = moment(new Date(teacher[6])).add(
      parseInt(reminder),
      "days"
    );
    let email = teacher[2];
    if (lastSubmittedDate.isValid()) {
      if (reminderDate.diff(new Date(), "days") === 0) {
        let body = "You can do so here: https://werkpaats.github.io/TimeSheet/";
        GmailApp.sendEmail(email, "Reminder to fill in hours", body, {
          htmlBody: body,
        });
      }
    }
